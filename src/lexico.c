...
int lexico_init_int() {
	...
	lexico_afd.estados_quantidade = 5;
...
int lexico_init_char() {
	...
	estado->transicoes_quantidade = 2;
	PCALLOC(estado->transicoes, estado->transicoes_quantidade);
	estado->transicoes[0].match = afd_transicao_match_interval_not;
	estado->transicoes[0].dados = "\'\\";
	estado->transicoes[0].estado = 3;
	...
	estado->transicoes[0].match = afd_transicao_match_interval;
	estado->transicoes[0].dados = "\'";
	estado->transicoes[0].estado = 4;

	estado = lexico_afd.estados + 4;
	estado->transicoes_quantidade = 0;
	estado->final = true;
	estado->parse = token_char_parse;
